...
│   ├── launcher.rs         # CLI引数 → worktree → tmux → claude起動
│   ├── notify.rs           # tmux bell + window rename による通知
│   ├── hooks.rs            # Claude Code hooks の生成・注入・除去
│   └── error.rs            # エラー型定義
```
...
`claude-mux launch` 実行時にworktreeの `<worktree>/.claude/settings.json` へ追加:
...
`claude-mux clean` で `_marker: "claude-mux-managed"` のエントリを除去。
